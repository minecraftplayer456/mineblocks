...
namespace engine::graphics {
    void assertVkResult(VkResult result)
    {
        if (result != VK_SUCCESS) {
            throw std::runtime_error("Vulkan error: " + stringifyVkResult(result));
        }
    }
    ...
    std::string stringifyVkResult(VkResult& result)
    {
        switch (result) {
            ...
            case VK_ERROR_OUT_OF_POOL_MEMORY:
                return "A allocation failed due to having no more space in the "
                       "descriptor pool";
            ...
            default:
                return &"Unknown Vulkan error: "[result];
        }
    }
} // namespace engine::graphics